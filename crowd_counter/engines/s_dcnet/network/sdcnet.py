...
class up(nn.Module):
    def __init__(
        self, up_in_ch, up_out_ch, cat_in_ch, cat_out_ch, if_convt=False
    ):
    ...
    def forward(self, x1, x2):
        ...
        x1 = F.pad(
            ...
            (
                diffX // 2,
                int(math.ceil(diffX / 2.0)),
                diffY // 2,
                int(math.ceil(diffY / 2.0)),
            ),
        )  # 3//2=1,3/2=1.5
...
def compute_rf(net):
    ...
    for m in net.modules():
        if isinstance(m, (nn.Conv2d, nn.MaxPool2d)):
            ...
            tmp_stride = (
                m.stride[0] if isinstance(m.stride, (tuple, list)) else m.stride
            )
...
class SDCNet_VGG16_classify(nn.Module):
    def __init__(
        ...
        """ SDCNet with VGG16 baseline
        Args:
            label_indice (torch.Tensor)

        Optional:
            div_times (int, default=2):

            load_weights (bool): Preload model weight with VGG16 or not
            freeze_bn (bool): Freeze Batch Norm or not
        """
        ...
        self.fc = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Conv2d(512, self.class_num, (1, 1)),
        )
        ...
        if load_weights:
            ...
            for name, params in list(pretrained_dict.items()):
                if "conv" in list(net_dict_name)[lay_num]:
                    net_dict[list(net_dict_name)[lay_num]] = pretrained_dict[
                        name
                    ]
                    lay_num = lay_num + 1
                else:
    ...
    def parse_merge(self, div_res):
        ...
        for cidx in range(self.div_times + 1):
            ...
            div_res["cls" + str(cidx)] = div_res["cls" + str(cidx)].max(
                dim=1, keepdim=True
            )[1]
