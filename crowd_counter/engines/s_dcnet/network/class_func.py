...
def class2count(count_map, label_indice):
    ...
    if IF_gpu:
        count_map, label_indice, cls_map = (
            count_map.cuda(),
            label_indice.cuda(),
            cls_map.cuda(),
        )
    ...
    for i in range(cls_num - 1):
        if IF_gpu:
            cls_map = (
                cls_map
                + (count_map >= label_indice[i])
                .cpu()
                .type(torch.LongTensor)
                .cuda()
            )
        else:
            cls_map = cls_map + (count_map >= label_indice[i]).cpu().type(
                torch.LongTensor
            )
    if not IF_ret_gpu:
        cls_map = cls_map.cpu()
...
def count2class(pre_cls, label_indice):
    """
    # --Input:
    ...
    pre_counts = torch.index_select(
        label2count, 0, pre_cls.cpu().type(torch.LongTensor)
    )
