""" Source code copy from https://github.com/weizheliu/Context-Aware-Crowd-Counting """
...
class ContextualModule(nn.Module):
    def __init__(self, features, out_features=512, sizes=(1, 2, 3, 6)):
        ...
        self.scales = nn.ModuleList(
            [self._make_scale(features, size) for size in sizes]
        )
    ...
    def forward(self, feats):
        ...
        weights = [
            self.__make_weight(feats, scale_feature)
...
class CANNet(nn.Module):
    def __init__(self, load_weights=False):
        ...
        self.backend = make_layers(
            self.backend_feat, in_channels=512, batch_norm=True, dilation=True
        )
